...
class CNN(object):
    ...
    def initialize(self):
        ...
        '''
        print 'featureMapSize:', featureMapSize
        print 'batchSize:', self.batchSize
        print 'self.patchSize:', self.patchSize
        print 'self.kernelSize:', self.kernelSizes
        print 'self.nkerns:', self.nkerns
        '''
    ...
    def predicta(self, path, threshold=0.5):
    ...
    def train_online(self, data):
        ...
        print 'validx:', np.shape(valid_x)
        print 'validy:', np.shape(valid_y)
        ...
        while (elapsed_time < self.trainTime)\
            ...
            if (iter + 1) % validation_frequency == 0 and n_valid_batches > 0:
                ...
                '''
                msg = 'iter %i, minibatch %i/%i, training error %.3f, validation error %.2f %%' % (iter, minibatch_index + 1, n_train_batches, minibatch_avg_costs[-1], this_validation_loss)
                ...
                #print minibatch_avg_costs[-1], '-', this_validation_loss, '/',self.best_validation_loss, len(np.where(probs==0)[0]), len(np.where(probs==1)[0])
        ...
        #if (error <= self.error or error < self.error_threshold) and (n_valid_batches == 0):
        '''
        if n_valid_batches == 0:
            save_on1 = (data.accuracy >= 0.9 and error <= self.error)
            save_on2 = (error <= self.error_threshold)
            if save_on1 or save_on2:
                self.save()
                self.error = error
        '''
        if n_valid_batches == 0:
